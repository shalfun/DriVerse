...
class LightningModelForTrain(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        prompt_emb = self.pipe.encode_prompt(text)
        print(video.shape)
        print(self.tiler_kwargs)

        my_device = torch.distributed.get_rank()
        self.pipe.vae = self.pipe.vae.to(my_device)
        latents = self.pipe.vae.encode(video.to(my_device), device=my_device,
        ...
        pil_image = self.to_pil((video[0, :, 0] + 1) / 2)
        image_feature = self.pipe.encode_image(pil_image, num_frames, height, width)
        clip_feature = image_feature["clip_feature"].to(self.device)[0]
        y = image_feature["y"].to(self.device)[0]
        prompt_emb["context"] = prompt_emb["context"][0].to(self.device)
        """

        # Data
        latents = batch["latents"].to(self.device)
        clip_feature = batch["clip_feature"].to(self.device)[0]
        y = batch["y"].to(self.device)[0]
        ...
        noise_pred = self.pipe.denoising_model()(
            noisy_latents, timestep=timestep, clip_feature=clip_feature, y=y, **prompt_emb, **extra_input,
